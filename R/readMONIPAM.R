...
#'
#' @usage read.MONIPAM(source.path,pam.pattern,save.path,site.lat,site.lon,local.tz,tz.summer,tz.winter,measure.time)
...
read.MONIPAM<-function(source.path,#folder where files stored
  ...
  preproces.moni<-raw.moni.unique.all %>%dplyr::select(-filename) %>% unique()
  ...
  preproces.dup<-
    preproces.moni[duplicated(preproces.moni%>% dplyr::select(-tree_num),fromLast = T)|
                     duplicated(preproces.moni%>% dplyr::select(-tree_num)),]

  if (isTRUE(nrow(preproces.dup)>0)){#when without considering 'tree_num' column,
    #the duplicated rows are exist

    preproces.nodup<-
      preproces.moni[-which(duplicated(preproces.moni %>% dplyr::select(-tree_num),
                                       fromLast = T)|
                              duplicated(preproces.moni %>% dplyr::select(-tree_num))),]
