...
#'
#' @return [plot.moni.diary] will not return/show the plotted figures in the end, instead, it will save the plotted figures to your folder directly.
#' @export
plot.moni.diary<-function(moni.data,
                          diary.data,
                          plot.title,
                          save.path
){
  ...
  lapply(levels(moni.data$plot.group),function(i){
    ...
    if (sum(na.omit(moni.data.onegroup$Yield))>0){#when there is data exit
      ...
      plot.onegroup<-
        lapply(levels(moni.data.onegroup$head_tree),function(j){
          ...
          if (length(na.omit(plot.data$Yield))<=2&
            ...
            p1<-ggplot(plot.data,aes(x=datetime))+
              geom_line(aes(y=scale.temp.a*temp_moni+scale.temp.b),
                        size=0.71,color='darkorchid',na.rm = T)+
              #geom_hline(aes(yintercept=scale.temp.b),color='plum4')+
              geom_line(aes(y=scale.par.a*par_moni+scale.par.b),
                        size=0.71,color='gold3',na.rm = T)+
              ...
              geom_point(aes(y=scale.Fm.a*F_+scale.Fm.b),size=0.5,color='grey30',na.rm = T)+
              geom_line(aes(y=scale.Fm.a*Fm_+scale.Fm.b),color='black',na.rm = T)+
              geom_point(aes(y=scale.Fm.a*Fm_+scale.Fm.b),color='black',size=0.5,na.rm = T)+
              scale_x_datetime(breaks = date_breaks("1 day"),
                               labels = date_format("%b-%d\n%H:%M"),
              ...
              p1<-p1+
                ...
                geom_text(data=na.omit(diary.group),
                          aes(x=datetime,y=max(plot.data$Yield,na.rm = T),
                              label=remark.plot),color='red',size=3)
            } else {
              p1<-p1
            }
