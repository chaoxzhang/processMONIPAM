...
scaleYield<-function(diurnal.para){
  ...
  maxmin.res<-
    ...
    dplyr::summarise(
      across(
        .col=c('Yield','Phi_fD','Phi_NPQ','Phi_NPQr','Phi_NPQs',
               'par_moni','temp_moni'),
  ...
  maxmin.res<-
    maxmin.res %>%
    mutate(scale.temp.a=(1-0.1)/(max.temp_moni-min.temp_moni)) %>%
    mutate(scale.temp.b=0.1-scale.temp.a*min.temp_moni) %>%
    mutate(scale.par.a=(1-0.1)/(max.par_moni-min.par_moni)) %>%
    mutate(scale.par.b=0.1-scale.par.a*min.par_moni)

  maxmin.res[sapply(maxmin.res, is.infinite)] <- NA
  maxmin.res[sapply(maxmin.res, is.nan)]<-NA
  maxmin.res[,c(19,21)][sapply(maxmin.res[,c(19,21)],is.na)]<-1
  maxmin.res[,c(20,22)][sapply(maxmin.res[,c(20,22)],is.na)]<-0
  return(maxmin.res)
}


#' Diurnal quantum yield parameters data visualization with dairy note and sunlight time marked in the figure
#'
#' This function will visualize MONI-PAM diurnal quantum yield parameters data (PhiPSII,PhiNPQ,PhifD) and PAR and temperature in one figure with 10 days as a group plotting for each head and the dairy note will be marked in the figure if the note file is available. At the same time, this function will also show  the sunlight time (colorful vertical lines) in the plot, sunrise and sunsetStart are 'indianred', solarNoon is 'goldenrod', and dawn and dusk are 'purple'
#'
#' @usage plotYield(diurnal.para,diary,diary.data,save.path)
...
plotYield<-function(diurnal.para,
  ...
  extreme.data<-scaleYield(diurnal.para)
          ...
          if (isTRUE(sum(na.omit(plot.data$Yield))>0&
            ...
            p1<-ggplot(plot.data,aes(x=datetime))+
              ...
              geom_vline(aes(xintercept=dawn),na.rm = T,
                         color='purple',linetype=2)+
              geom_line(aes(y=TtoYield(temp_moni)),color='darkorchid',na.rm = T,alpha=0.5)+
              ...
              geom_line(aes(y=Phi_fD),color='orange',na.rm = T)+
              geom_point(aes(y=Phi_fD),color='orange',size=0.5,na.rm = T)+
              ...
              scale_y_continuous(
                sec.axis = sec_axis(~.,name="<span style='color: darkred;'>&#934;NPQ or <span style='color: orange;'>&#934;f,D"),
                name = bquote(atop(paste(Phi,'PSII'),
                                   .(j))))+
              theme_bw()+
              theme(legend.position = 'none',
            ...
            p23.fc<-function(p23.color){
              ggplot(plot.data,aes(x=datetime))+
                ...
                geom_line(aes(y=Phi_fD),color='white',na.rm = T)+
                geom_point(aes(y=Phi_fD),size=1,color='white',na.rm = T)+
