...
scaleQuench<-function(diurnal.para){
  ...
  maxmin.res<-
    ...
    dplyr::summarise(
      across(
        .col=c('qLT','qLr','par_moni','temp_moni'),
  ...
  maxmin.res<-
    maxmin.res %>%
    mutate(scale.temp.a=(max.qLr-min.qLr)/(max.temp_moni-min.temp_moni)) %>%
    mutate(scale.temp.b=min.qLr-scale.temp.a*min.temp_moni) %>%
    mutate(scale.par.a=(max.qLr-min.qLr)/(max.par_moni-min.par_moni)) %>%
    mutate(scale.par.b=min.qLr-scale.par.a*min.par_moni)


  maxmin.res[sapply(maxmin.res, is.infinite)] <- NA
  maxmin.res[sapply(maxmin.res, is.nan)]<-NA
  maxmin.res[,c(13,15)][sapply(maxmin.res[,c(13,15)],is.na)]<-1
  maxmin.res[,c(14,16)][sapply(maxmin.res[,c(14,16)],is.na)]<-0
  return(maxmin.res)
}


#' Diurnal quenching parameters data visualization with dairy note and sunlight time marked in the figure
#'
#' This function will visualize MONI-PAM diurnal quenching parameters data (qLT and qLr) and PAR and temperature in one figure with 10 days as a group plotting for each head and the dairy note will be marked in the figure if the note file is available. At the same time, this function will also show  the sunlight time (colorful vertical lines) in the plot, sunrise and sunsetStart are 'indianred', solarNoon is 'goldenrod', and dawn and dusk are 'purple'
#'
#' @usage plotQuench(diurnal.para,diary,diary.data,save.path)
    ...
    if (isTRUE(sum(na.omit(diurnal.para.onegroup$Yield))>0)){#when there is data exsit
      ...
      plot.onegroup<-
        lapply(levels(diurnal.para.onegroup$head_tree),function(j){
          ...
          TtoqLr<-function(x){scale.data$scale.temp.a*x+scale.data$scale.temp.b}
          PARtoqLr<-function(x){scale.data$scale.par.a*x+scale.data$scale.par.b}
          ...
          if (isTRUE(sum(na.omit(plot.data$Yield))>0&
            ...
            p1<-ggplot(plot.data,aes(x=datetime))+
              ...
              geom_vline(aes(xintercept=dawn),na.rm = T,
                         color='purple',linetype=2)+
              geom_line(aes(y=TtoqLr(temp_moni)),color='darkorchid',na.rm = T)+
              geom_line(aes(y=PARtoqLr(par_moni)),color='yellow3',na.rm = T)+
              geom_line(aes(y=qLr),color='black',na.rm = T)+
              geom_point(aes(y=qLr),color='black',size=0.5,na.rm = T)+
              geom_line(aes(y=qLT),color='forestgreen',na.rm = T)+
              geom_point(aes(y=qLT),size=0.5,color='forestgreen',na.rm = T)+
              scale_x_datetime(breaks = date_breaks("1 day"),
                               labels = date_format("%b-%d\n%H:%M"),
                               limits = c(range(na.omit(diurnal.para.onegroup$datetime))[1],
                                          range(na.omit(diurnal.para.onegroup$datetime))[2]))+
              scale_y_continuous(
                sec.axis = sec_axis(~.,name="qLr"),
                name = bquote(atop(paste('qLT'),.(j))))+
              theme_bw()+
              theme(legend.position = 'none',
                    title = element_text(size=12,color='black',face='bold'),
              ...
              if (isTRUE(length(na.omit(diary.group$remark.plot))>0)) {
                ...
                p1<-p1+
                  geom_point(data=na.omit(diary.group),
                             aes(x=datetime,y=max(plot.data$qLT,na.rm = T)-0.05),
                             color='red',size=4,na.rm=T)+
                  geom_text(data=na.omit(diary.group),
                            aes(x=datetime,y=max(plot.data$qLT,na.rm = T),
            ...
            p23.fc<-function(p23.color){
              ggplot(plot.data,aes(x=datetime))+
                geom_line(aes(y=TtoqLr(temp_moni)),linewidth=0.1,color='white',na.rm = T)+
                geom_line(aes(y=PARtoqLr(par_moni)),linewidth=0.1,color='white',na.rm = T)+
                geom_line(aes(y=qLT),color='white',na.rm = T)+
                geom_point(aes(y=qLT),size=1,color='white',na.rm = T)+
                geom_line(aes(y=qLr),color='white',na.rm = T)+
                geom_point(aes(y=qLr),size=1,color='white',na.rm = T)+
                theme_minimal()+
                theme(legend.position = 'none',
                      axis.text.y.right = element_text(size = 15,color = p23.color),
                      axis.text.y.left = element_blank(),
