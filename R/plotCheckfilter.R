...
#'
#' @usage plotCheckfilter(newdata, rawdata,save.path,diary.data)
...
plotCheckfilter<-function(newdata,
  ...
  newdata<-formatMONIdata(moni.data=newdata)
  rawdata<-formatMONIdata(moni.data=rawdata)
  ...
  lapply(levels(rawdata$plot.group),function(i){
    ...
    if (isTRUE(sum(na.omit(rawdata.onegroup$Yield))>0&
      ...
      plot.onegroup<-
        lapply(levels(rawdata.onegroup$head_tree),function(j){
          ...
          if (isTRUE(diary==T&length(na.omit(bottom.data$Yield))<=2& # when there is no Yield data,
              ...
              plot.moni.figure<-
                ...
                ylim(0.4,0.7)
              } else if (isTRUE((sum(na.omit(bottom.data$Yield))>0&
                ...
                p1<-ggplot(bottom.data,aes(x=datetime))+
                  ...
                  geom_vline(aes(xintercept=dawn),na.rm = T,
                             color='purple',linetype=2)+
                  geom_line(aes(y=TtoYield(temp_moni)),color='darkorchid',na.rm = T)+
                  geom_line(aes(y=PARtoYield(par_moni)),color='yellow3',na.rm = T)+
                  geom_line(aes(y=FtoYield(F_)),color='grey30',na.rm = T)+
                  geom_point(aes(y=FtoYield(F_)),size=1,color='grey30',na.rm = T)+
                  geom_point(data=top.data0,aes(y=FtoYield(F_)),size=1,color='orangered',na.rm = T)+
                  geom_point(data=top.data2,aes(y=FtoYield(F_)),size=1,color='grey30',na.rm = T)+
                  geom_line(aes(y=FtoYield(Fm_)),color='black',na.rm = T)+
                  geom_point(aes(y=FtoYield(Fm_)),color='black',size=1,na.rm = T)+
                  geom_point(data=top.data0,aes(y=FtoYield(Fm_)),color='deeppink',size=1,na.rm = T)+
                  geom_point(data=top.data2,aes(y=FtoYield(Fm_)),color='black',size=1,na.rm = T)+
                ...
                p23.fc<-function(p23.color){
                  ...
                  geom_line(aes(y=TtoYield(temp_moni)),linewidth=0.1,color='white',na.rm = T)+
                  geom_line(aes(y=PARtoYield(par_moni)),linewidth=0.1,color='white',na.rm = T)+
                  ...
                  geom_point(aes(y=FtoYield(F_)),size=0.5,color='white',na.rm = T)+
                  geom_point(data=top.data0,aes(y=FtoYield(F_)),size=0.5,color='white',na.rm = T)+
                  geom_point(data=top.data2,aes(y=FtoYield(F_)),size=0.5,color='white',na.rm = T)+
                  geom_line(aes(y=FtoYield(Fm_)),color='white',na.rm = T)+
                  geom_point(aes(y=FtoYield(Fm_)),color='white',size=0.5,na.rm = T)+
                  geom_point(data=top.data0,aes(y=FtoYield(Fm_)),color='white',size=0.5,na.rm = T)+
                  geom_point(data=top.data2,aes(y=FtoYield(Fm_)),color='white',size=0.5,na.rm = T)+
                  theme_minimal()+
                  theme(legend.position = 'none',
