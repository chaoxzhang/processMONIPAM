...
#'
#' @usage moni.diary.suntime.plot(moni.data,diary.data,plot.title,save.path)
...
#' @export
plotMONIdiarySuntime<-function(moni.data,
                               diary,
                               diary.data,
                               plot.title,
                               save.path
  ...
  lapply(levels(moni.data$plot.group),function(i){
    ...
    if (isTRUE(sum(na.omit(moni.data.onegroup$Yield))>0)){#when there is data exsit
